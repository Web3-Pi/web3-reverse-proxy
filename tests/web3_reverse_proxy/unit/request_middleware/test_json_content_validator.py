...
from web3_reverse_proxy.core.rpc.request.middleware.jsonrpcmiddlewares.jsoncontentvalidator import (
...
class AcceptJSONRPCContentReaderTests(TestCase):
    ...
    def _pass_request(json_reader, request):
        with patch(
            "web3_reverse_proxy.core.rpc.request.middleware.jsonrpcmiddlewares.jsoncontentvalidator.Config",
    ...
    def test_requests_missing_members_should_fail(self):
        ...
        for payload in RPCCalls.generate_missing_member_calls():
            with self.subTest(f'{payload.get("method")} -> {payload.get("params")}'):
                _, response = self._pass_request(
                    json_reader, self._make_request(payload)
                )
    ...
    def test_request_format_validation(self):
        test_data = [
            ...
            [{"method": "net_version", "id": 0, "params": "", "jsonrpc": "2.0"}, False],
            [
                {"method": "net_version", "id": 0, "params": None, "jsonrpc": "2.0"},
                False,
            ],
            [
                {"method": "net_version", "id": 0, "params": 256, "jsonrpc": "2.0"},
                False,
            ],
            [
                {"method": "net_version", "id": 0, "params": True, "jsonrpc": "2.0"},
                False,
            ],
            [
                {"method": "net_version", "id": "0", "params": [], "jsonrpc": "2.0"},
                True,
            ],
            [
                {"method": "net_version", "id": "foo", "params": [], "jsonrpc": "2.0"},
                True,
            ],
            [
                {"method": "net_version", "id": None, "params": [], "jsonrpc": "2.0"},
                True,
            ],
            [
                {"method": "net_version", "id": [], "params": [], "jsonrpc": "2.0"},
                False,
            ],
            [
                {"method": "net_version", "id": {}, "params": [], "jsonrpc": "2.0"},
            ...
            [
                {"method": "net_version", "id": True, "params": [], "jsonrpc": "2.0"},
                False,
            ],
        ]
        json_reader = AcceptJSONRPCContentReader()
    ...
    def test_passes_invalid_requests_when_disabled(self):
        ...
        ] + RPCCalls.generate_non_alphanumeric_input_calls()

        for payload in test_data:
            with self.subTest(payload=payload):
                with patch(
                    "web3_reverse_proxy.core.rpc.request.middleware.jsonrpcmiddlewares.jsoncontentvalidator.Config",
                    JSON_RPC_REQUEST_PARSER_ENABLED=False,
                ):
                    request = self._make_request(payload)
        ...
        for payload in test_data:
            with self.subTest(payload=payload):
                with patch(
                    ...
                    _, response = json_reader.read_request(
                        Mock(ClientSocket), self._make_request(payload)
                    )
