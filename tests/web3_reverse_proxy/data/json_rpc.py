...
VALID_METHOD_PARAMS = {
    ...
    "eth_getBlockByHash": [
        ["0x3f07a9c83155594c000642e7d60e8a8a00038d03e9849171a05ed0e2d47acbb3", False]
    ],
    ...
    "eth_getStorageAt": [
        ["0xE592427A0AEce92De3Edee1F18E0157C05861564", "0x0", "latest"]
    ],
...
class RPCCalls:
    ...
    @classmethod
    def create_json_payload_without_member(
        cls, method: str, params: List[Any], member: str
    ) -> Dict[str, Any]:
        ...
        for member in ["jsonrpc", "method", "id"]:
            ...
            for method in methods:
                for params in VALID_METHOD_PARAMS[method]:
                    payload = cls.create_json_payload_without_member(
        ...
        for method in methods:
            for params in VALID_METHOD_PARAMS[method]:
                if params is not None and len(params) > 0:
                    non_alnum_index = methods.index(method) % (
                        len(non_alphanumeric_set) - 1
                    )
    ...
    @classmethod
    def generate_stress_test_calls(
        cls, method="eth_sendTransaction", count=1000
    ) -> List[Dict[str, Any]]:
        return [
            cls.create_json_payload(method, VALID_METHOD_PARAMS[method][-1])
        ] * count
