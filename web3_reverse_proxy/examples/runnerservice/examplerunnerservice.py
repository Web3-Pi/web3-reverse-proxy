...
from web3_reverse_proxy.service.providers.serviceprovider import (
    ServiceComponentsProvider,
)
from web3_reverse_proxy.service.providers.statemanagerprovider import (
...
class ExampleRunnerService:
    ...
    def __init__(self, console_buffer: StringIO):
        ...
        self.state_manager = StateManagerProvider.create_state_manager(
            console_buffer, skip_persistent_db=True
        )
    ...
    def __print_pre_init_info(cls):
        print(
            f"Starting {Config.SERVICE_NAME}, version {Config.SERVICE_VER} - EXAMPLE MODE (pickle DB is not active)"
        )
    ...
    def run_forever(self, handler: EndpointsHandler):
        upnp_service = UPnPServiceProvider.create_basic_upnp_service(
            Config.PROXY_LISTEN_PORT, Config.ADMIN_LISTEN_PORT
        )
        ...
        StateManagerProvider.close_state_manager(
            self.state_manager, skip_persistent_db=True
        )
