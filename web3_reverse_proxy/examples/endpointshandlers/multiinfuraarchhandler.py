...
class MultiInfuraArchEndpointHandler(ThreadedEndpointHandler):
    def __init__(self, name: str, conn_descr: EndpointConnectionDescriptor, state_updater: StateUpdater):
        ...
        self.endpoint_non_arch = RPCEndpoint.create(name + " non-arch", conn_descr, state_updater)
        self.non_arch_responses = {}

    def add_request(self, cs: ClientSocket, req: RPCRequest) -> None:
        if req.method != "eth_getBalance":
            self.non_arch_responses[cs] = self.endpoint_non_arch.handle_request_response_roundtrip(req)
    ...
    def create(cls, name_geth: str, geth_descr: EndpointConnectionDescriptor,
               ...
               state_updater: StateUpdater) -> MultiInfuraArchEndpointHandler:
        ...
        instance.add_request_consumer(Queues.IN_0, RPCEndpoint.create(name_infura + " arch", infura_descr, state_updater))
