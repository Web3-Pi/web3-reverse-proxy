...
class MultiPriorityComplexEndpointHandler(ThreadedEndpointHandler):
    ...
    def add_request(self, cs: ClientSocket, req: RPCRequest) -> None:
        if req.priority == 0 and req.method == "net_version":
            self.fast_responses[cs] = self.endpoint_fast.handle_request_response_roundtrip(req)
    ...
    def create(cls, name_geth: str, geth_descr: EndpointConnectionDescriptor,
               ...
               state_updater: StateUpdater) -> MultiPriorityComplexEndpointHandler:

        instance = MultiPriorityComplexEndpointHandler(name_geth, geth_descr, state_updater)
