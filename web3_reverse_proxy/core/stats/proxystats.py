...
class RPCProxyStats:
    ...
    def update(self, no_new_clients: int, no_processed_requests: int, no_disconnected_clients: int) -> None:
    ...
    def is_ready_to_update_display(self):
        ...
        count_cond = (self.request_num + self.clients_num) // self.stats_print_delta > self.prev_stats_print_marker

        return time_cond and count_cond

    def display_rudimentary_stats(self) -> None:
        self.prev_stats_print_marker = (self.request_num + self.clients_num) // self.stats_print_delta
        self.last_update = time.time()

        print(f"\rProcessed {self.request_num} requests from {self.clients_num} connections "
              f"and disconnected {self.disconnected_num} clients", end="")
