...
class TrustedNodeVerifier:
    def __init__(self, trusted_node_url: str):
        self.__trusted_node_connection_descriptor = EndpointConnectionDescriptor.from_url(trusted_node_url)
        self.__logger = get_logger("TrustedNodeVerifier")

        self.__comparison_rules = {
            "web3_clientVersion": ["content_length", "body"],
            "eth_getLogs": ["content_length"],
    ...
    def verify(self, req: RPCRequest, node_response_raw: bytes, trusted_response_raw: bytes) -> bool:
        try:
            self.__logger.debug("Verifying response from trusted node...")

            comparison_types = self.__comparison_rules.get(req.method, ["content_length"])
            ...
            for comparison_type in comparison_types:
                ...
                if comparison_method is None:
                    self.__logger.error(f"Comparison method '{comparison_type}' is not implemented.")
                    raise NotImplementedError(f"Comparison method '{comparison_type}' is not implemented.")
    ...
    def _compare_content_length(self, node_response_raw: bytes, trusted_response_raw: bytes) -> bool:
    ...
    def _compare_body(self, node_response_raw: bytes, trusted_response_raw: bytes) -> bool:
        headers_node = self.__parse_headers(node_response_raw)
        headers_trusted = self.__parse_headers(trusted_response_raw)
